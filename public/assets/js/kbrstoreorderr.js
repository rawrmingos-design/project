...
function showToast(e, a = "error") {
    ...
    d && m && h ? h ? $.ajax({
            ...
            $(".load").removeClass("show"), f.status ? Swal.fire({
                ...
                if (f.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        success: function(e) {
                            $(".load").removeClass("show"), e.status ? (showToast("Berhasil membuat pesanan!", "success"), window.location = `/id/invoices/${e.order_id}`) : showToast("Terdapat Kesalahan!", "error")
                        },
                        error: function(e) {
        ...
        error: function(e) {
            $(".load").removeClass("show"), 422 === e.status ? Swal.fire({
                title: "Oops...",
                text: "Pastikan anda sudah mengisi semua data yang diperlukan.",
                icon: "error"
            }) : Swal.fire({
                title: "Oops...",
                text: "Terjadi kesalahan. Silakan coba lagi. Pastikan User ID Benar",
                icon: "error"
            })
        }
    ...
    $.ajax({
        ...
        error: function(e) {
            setTimeout(o, 4e3), showToast(a.responseJSON.message, "error")
        }
