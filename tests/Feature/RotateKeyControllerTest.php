...
class RotateKeyControllerTest extends TestCase
{
    ...
    private function createResellerUserWith2FA($with2fa = true)
    {
        ...
        $user = User::factory()->create([
            ...
            'api_key' => Hash::make('old_live_key'),
            'api_key_hint' => 'old_...hint',
            'sandbox_api_key_hash' => Hash::make('old_sandbox_key'),
            'sandbox_api_key_hint' => 'old_...sbx',
        ]);
    ...
    public function test_reseller_can_rotate_live_api_key_and_receive_plain_text_once()
    {
        ...
        $this->assertStringStartsWith('live_', $rawKey);
        ...
        $user->refresh();
        $this->assertEquals($hint, $user->api_key_hint);
        $this->assertEquals(substr($rawKey, 0, 8), $user->api_key_prefix);
        $this->assertTrue(Hash::check($rawKey, $user->getRawOriginal('api_key')));

        // raw key NOT stored in plain text
        $this->assertNotEquals($rawKey, $user->getRawOriginal('api_key'));
    ...
    public function test_reseller_can_rotate_sandbox_api_key_and_receive_plain_text_once()
    {
        ...
        $this->assertStringStartsWith('sbx_', $rawKey);
        $this->assertStringStartsWith('...', $hint);

        $user->refresh();
        $this->assertEquals($hint, $user->sandbox_api_key_hint);
        $this->assertTrue(Hash::check($rawKey, $user->sandbox_api_key_hash));
